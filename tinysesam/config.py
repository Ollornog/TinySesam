...
class TinySesamConfig:
    ...
    # Die E-Mail aus dem ID-Token ist eine Behauptung des Providers über ein fremdes Postfach.
    # Belegt ist sie nur mit dem Standard-Claim `email_verified` (OIDC Core 5.1); fehlt er, gilt
    # sie als unbestätigt. Mit True (Vorgabe) wird eine unbelegte Adresse gar nicht erst ins Konto
    # übernommen — sonst trägt sie über `admin_identifiers` und den Header `Remote-Email`
    # Entscheidungen, die niemand geprüft hat. False nur für IdPs, die den Claim nicht schicken
    # (z.B. Entra ID) UND deren Adressen der Betreiber selbst verantwortet; das Admin-Recht aus
    # `admin_identifiers` bleibt auch dann an den Beleg gebunden.
    ...
    # Leer = aus dem Request abgeleitet, aber NUR wenn dessen Host in trusted_redirect_hosts
    # steht oder Loopback ist — sonst bricht der jeweilige Vorgang ab (es geht keine Mail
    # hinaus). Grund: Der Host-Header ist eine Eingabe des Anfragenden; ohne diese Prüfung
    # konnte ein Angreifer den Reset-Link in der Mail des Opfers auf seinen Server umbiegen.
    base_url: str = ""
