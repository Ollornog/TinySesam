...
| **Datenbank gesperrt** (zweiter Schreiber: weiterer Worker, `tinysesam` auf der Kommandozeile, Sicherung) | Die Anfrage wartet bis zu **10 s** (`Store.BUSY_TIMEOUT_MS`, B6-11), dann 500 (`database is locked`). | die 500 steht im Log des ASGI-Servers. `/healthz` wartet ebenso und meldet erst nach 10 s 503 — der HEALTHCHECK des Abbilds wartet deshalb 12 s. |
