...
| **Datenbank gesperrt** (zweiter Schreiber: weiterer Worker, `tinysesam` auf der Kommandozeile, Sicherung) | Die Anfrage wartet bis zu **10 s** (`Store.BUSY_TIMEOUT_MS`, B6-11), dann 500 (`database is locked`). | nichts Eigenes — die 500 steht im Log des ASGI-Servers |
...
| **SMTP / Mailer** | Magic-Link und „Passwort vergessen": dieselbe Erfolgsseite wie immer (keine Konto-Enumeration), aber keine Mail. Registrierung mit Bestätigungsmail: 500 (B6-5). | Audit: `magic_send_error` bzw. `reset_send_error`. Das Token der nicht zugestellten Mail bleibt bis zu seiner Frist gültig (B6-12). |
...
| **LDAP-Server** | „Anmeldedaten falsch" (401) — nicht zu unterscheiden von einem Tippfehler. | **Achtung:** Der Fehlschlag zählt als Fehlversuch (`record_login`). Ein längerer Ausfall treibt so jedes LDAP-Konto, das es versucht, in die Kontosperre. Nach dem Ausfall: `tinysesam unlock`. |
...
| **Systemuhr springt zurück** (NTP, Pi ohne Pufferbatterie, VM-Snapshot) | nichts — Fristen laufen normal weiter | Warnzeile `Die Systemuhr steht … hinter der zuletzt benutzten Zeit`. TinySesam zählt monoton weiter und liest den Stand beim Start aus der Datenbank; abgelaufene Sitzungen, Einmal-Token und Step-ups leben nicht wieder auf (B6-9). Sprang die Uhr einmal falsch nach **vorn**, bleiben Zeitstempel dort, bis die Wanduhr aufholt. |
...
| Admin | Konto sperren | `POST <admin_path>/api/users/{id}/disable` — beendet alle Sitzungen, widerruft API-Keys und verwirft offene Einmal-Token (Anmelde-, Bestätigungs-, Reset-Link) |
| Code | dasselbe ohne HTTP | `auth.store.list_sessions(user_id)`, `delete_session_by_handle(handle)`, `delete_user_sessions(user_id)`, `delete_user_sessions_except(user_id, handle)` |
